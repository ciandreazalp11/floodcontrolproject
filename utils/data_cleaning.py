...
def process_df(df: pd.DataFrame,
    ...
    # Combine Date/Day/Year if present
    ...
    # find numeric water column if None
    if water_col is None:
        numeric_cols = df.select_dtypes(include=[np.number]).columns.tolist()
        if not numeric_cols:
            raise ValueError("No numeric column detected to use as water level.")
